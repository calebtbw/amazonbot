...
GLOBAL_CONFIG_FILE = "config/amazonbot.conf"
...
class GlobalConfig:
    def __init__(self) -> None:
        ...
        self.amazonbot_config = self.global_config.get("AMAZONBOT")
    ...
    def get_amazonbot_config(self):
        return self.amazonbot_config

    def get_browser_profile_path(self):
        if not self.profile_path:
            self.profile_path = os.path.join(
                os.path.dirname(os.path.abspath("__file__")),
                self.global_config["AMAZONBOT"].get("profile_name", ".profile-amz"),
            )
