...
class Amazon:
    ...
    def run(self, delay=DEFAULT_REFRESH_DELAY, test=False):
        ...
        log.info(f"AmazonBot ran for {runtime} seconds.")
        time.sleep(10)  # add a delay to shut stuff done

    def fail_to_checkout_note(self):
        log.info(
            "It's likely that the product went out of stock before AmazonBot could checkout."
        )
        log.info(
            "Also verify that your default shipping and payment options are selected and work correctly."
        )
        log.info("AmazonBot WILL NOT select shipping and payment options for you.")
    ...
    def navigate_pages(self, test):
        ...
        if title in amazon_config["SIGN_IN_TITLES"]:
            ...
            if self.shipping_bypass:
                ...
                log.warning(
                    "Landed on address selection screen.  AmazonBot will NOT select an address for you.  "
            ...
            log.warning(
                "AmazonBot is not sure what page it is on - will attempt to resolve."
            )
            ###################################################################
            ...
            if element:
                log.info(
                    "AmazonBot thinks it completed the purchase, please verify ASAP"
                )
                self.send_notification(
                    message="AmazonBot may have made a purchase, please confirm ASAP",
            ...
            if element:
                if self.do_button_click(
                    button=element,
                    clicking_text="AmazonBot thinks it is seeing a Prime Offer, attempting to click No Thanks",
                    fail_text="AmazonBot could not click No Thanks button",
                    log_debug=True,
                ):
                    return
            ...
            if self.driver.title != title:
                log.info(
                    "AmazonBot thinks user intervened in time, will now continue running"
                )
                return
            else:
                log.warning(
                    "AmazonBot does not think the user intervened in time, will attempt other methods to continue"
                )
            ...
            log.error(
                "AmazonBot could not navigate current page, refreshing and returning to handler"
            )
    ...
    def handle_shipping_page(self):
        ...
        try:
            ...
            pass
        if element:
            log.warning("AmazonBot thinks it needs to pick a shipping address.")
        ...
        log.error("AmazonBot cannot find a button to click on the shipping page")
    ...
    def show_config(self):
        ...
        if self.shipping_bypass:
            log.warning(f"{'=' * 50}")
            log.warning(f"--AmazonBot will attempt to choose shipping address.")
...
def get_alt_shipping_costs(tree, free_shipping_string) -> Price:
    ...
    if shipping_node.tag == "div":
        ...
        if len(shipping_spans) > 0:
            ...
            log.error(
                f"Unable to locate price.  Assuming 0.  Found this: '{shipping_span_text}'  Consider reporting to Caleb T."
            )
    return FREE_SHIPPING_PRICE
